...
class ContainerIsolation implements IsolationProvider { id='isolation.container-test'; kind='isolation' as const; async prepare(input:{workspace:{id:string}}): Promise<ExecutionEnvironment> { return { id: `container:${input.workspace.id}`, kind: 'container', workspacePath: '/container/workspace', description: 'test container' }; } }
