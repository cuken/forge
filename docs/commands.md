...
Accepts the provider-neutral change set for a succeeded run whose task is `reviewing`, then marks the task `done` only when the provider returns `accepted` or `empty`. Validation gates run first; acceptance is blocked if any gate fails, if the change-set provider returns a `blocked` result with a human-readable reason, or if it returns the typed `merge-conflict` result.
...
The built-in Git worktree implementation first checks the project checkout that will receive the merge. `forge runs accept` always renders provider result states as one concise line without stack traces: `accepted <run>: <message>` or `empty <run>: <message>` exit 0 and mark the task `done`; `blocked <run>: <message>` or `conflict <run>: <message>` exit non-zero, record the provider reason in run acceptance metadata, and leave the task in `reviewing` for a human to fix and retry. With `--dry-run`, successful output is prefixed with `dry-run `. If the target checkout has uncommitted changes, Forge prints `blocked <run>: Cannot accept change set: project checkout has uncommitted changes`. Otherwise it stages and commits workspace changes on the run branch, then merges that branch into the project checkout. If Git reports merge conflicts, Forge prints `conflict <run>: Cannot accept change set: merge conflict in README.md`. Successful, blocked, or merge-conflict acceptance records acceptance metadata on the run record; validation results are recorded for both normal and dry-run accepts.

After an `accepted` or `empty` result, Forge looks up any linked workstream item (the provider item associated with the task or the `Workstream item:` task description marker) and invokes the generic `WorkstreamCompletionProvider`. That update receives provider-neutral completion metadata and is treated as part of the external acceptance workflow: if the tracker update throws, `forge runs accept` exits non-zero after the run/task acceptance has already been persisted. Fix the tracker/provider problem and retry; completion providers should make repeated calls idempotent by recognizing the accepted run id or task id.

For linked workstream completion audits, acceptance uses the accepted run as the canonical source of truth: `acceptedRunId` is always included, the commit reference is the change-set provider's accepted-result reference (for `change-set.git-worktree`, the target checkout's resulting `HEAD` SHA/id and the accepted run branch), and `sync` is included only when a provider reports push/PR metadata. Unsynced accepts leave `sync` absent rather than guessing from local commits, remotes, or branch names.
