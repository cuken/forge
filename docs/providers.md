...
- `IsolationRuntimeInspector` — optional isolation capability that reports whether a recorded execution environment still exists and whether an agent process is alive; Forge uses it to surface stale runs in `forge status` and to auto-recover orphaned task containers before sweeps/runs.
