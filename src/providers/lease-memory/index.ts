...
import { LeaseConflictError, leaseScopeKey, type LeaseHandle, type LeaseProvider } from '../../core/lease.js';
...
export class MemoryLeaseProvider implements LeaseProvider {
  ...
  async acquire(input: { task: Task; scopes: TaskResourceScope[] }): Promise<LeaseHandle> {
    ...
    if (conflict) throw new LeaseConflictError(`resource scope '${conflict}' is already leased by task ${this.held.get(conflict)}`, conflict, this.held.get(conflict));
